...
int TempletManager::CreateTaskTemplet(TASK_ITEM *task_item)
{
    ...
    while(1)
    {
        ...
        int rfret = av_read_frame(pInputFormatContext, pInPack);
        if(rfret < 0)
        {
        ...
        len = avcodec_decode_video2(pInputCodecContext, ptrOutFrame, &nComplete, pInPack);
        //判断是否是关键帧
#if 0
        ...
        if(nComplete > 0)
        {
            ...
            if((cnt_frameRate % frameRate) < cnt_frequency )
            {
                if(nFrame< realDuration)
                {
                    ...
                    ///////////////记录日志////////////////
                    sprintf(buff_temp,"bmp_fullPath:%s, feature_fullPath:%s",bmp_fullPath,feature_fullPath);
                    g_templet_logwrite.PrintLog(MyLogger::INFO,"%s",buff_temp);
                    ///////////////////////////////////////
...
int TempletManager::DeleteTemplet_list(TEMPLET_LIST *pTemplet_list)
{
    ...
    if(pTemplet_list !=NULL)
    {
        for(TEMPLET_LIST ::iterator itTemplet = pTemplet_list->begin(); itTemplet != pTemplet_list->end(); itTemplet++)
        {
            ...
            for(PICTURE_LIST ::iterator itPicture = (*itTemplet)->picture_list.begin();
                itPicture != (*itTemplet)->picture_list.end(); itPicture++)
            {
            ...
            if(NULL != (*itTemplet)->dstVideoWidth)
            {
                delete [] (*itTemplet)->dstVideoWidth;
                (*itTemplet)->dstVideoWidth=NULL;
            }

            if(NULL != (*itTemplet)->dstVideoHeight)
            {
                delete [] (*itTemplet)->dstVideoHeight;
                (*itTemplet)->dstVideoWidth=NULL;
            }
